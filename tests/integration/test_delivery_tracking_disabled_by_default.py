...
def test_delivery_tracking_disabled_by_default(monkeypatch, tmp_path):
    ...
    assert "/track/click/" not in sent_messages[0]["html_body"]
    assert "/track/open/" not in sent_messages[0]["html_body"]
...
def test_delivery_open_tracking_enabled_without_click_rewriting(monkeypatch, tmp_path):
    main = importlib.import_module("main")
    ...
    config_path = write_temp_config(
        ...
        overrides={
            ...
            "email": {
                "digest_recipients": ["tracking@example.com"],
                "digest_subject": "Pixel Only Digest",
            },
            "additional_sources": {"enabled": True, "hours": 48},
            "limits": {
                "select_top_stories": 2,
    ...
    assert result["status"] == "completed"
    assert len(sent_messages) == 1
    assert "/track/click/" not in html_body
    assert "/track/open/" in html_body

    open_path = html_body.split('src="', 1)[1].split('"', 1)[0]
    client = admin_app.app.test_client()
    open_response = client.get(open_path.removeprefix("http://curator.test"))
