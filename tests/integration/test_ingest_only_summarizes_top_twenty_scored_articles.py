...
def test_ingest_only_summarizes_top_twenty_scored_articles(tmp_path):
    ...
    summarized = [story for story in stories if str(story.get("summary_body", "")).strip()]
    unsummarized = [story for story in stories if not str(story.get("summary_body", "")).strip()]
