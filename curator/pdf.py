...
def _story_payloads(render_payload) -> Iterable[dict]:
    for entry in flatten_render_payload(render_payload):
        ...
        yield {
            ...
            "display_timestamp": str(entry.get("display_timestamp", "") or "").strip()
            or format_story_timestamp(str(entry.get("published_at", "") or "")),
            "body_blocks": [str(block) for block in normalized.get("other_paragraphs", []) if str(block).strip()]
            or _text_blocks(str(normalized.get("body", "") or fallback_body)),
            "key_takeaways": [str(item).strip() for item in normalized.get("key_takeaways", []) if str(item).strip()],
    ...
    *,
    ...
    if story_payloads:
        for index, story in enumerate(story_payloads, start=1):
            ...
            if metadata:
                elements.append(Paragraph(escape(metadata), meta_style))
            elements.append(Paragraph(escape(story["title"]), heading_style))
            for paragraph in story["body_blocks"] or ["No summary."]:
                elements.append(Paragraph(escape(paragraph), body_style))
            if story["key_takeaways"]:
                elements.append(Paragraph("KEY TAKEAWAYS", label_style))
                for takeaway in story["key_takeaways"]:
                    elements.append(Paragraph(escape(f"- {takeaway}"), body_style))
            if story["why_this_matters"]:
                elements.append(Paragraph("WHY THIS MATTERS", label_style))
                elements.append(Paragraph(escape(story["why_this_matters"]), body_style))
            if story["url"]:
                elements.append(Paragraph(escape(f"Read original: {story['url']}"), link_style))
