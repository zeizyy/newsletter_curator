...
def build_daily_runner_script(*, repo_dir: Path, env_file: Path, uv_bin: str) -> str:
    return "\n".join(
        [
            ...
            "set +a",
            "",
            "manual_weekly_digest=0",
            'for arg in "$@"; do',
            '  if [[ "$arg" == "--weekly-digest" ]]; then',
            "    manual_weekly_digest=1",
            "  fi",
            "done",
            "",
            'current_weekday="$(TZ=America/Los_Angeles date +%u)"',
            'if [[ "$current_weekday" == "7" ]] && [[ "$manual_weekly_digest" -ne 1 ]]; then',
            ...
            "set +e",
            f"{quote(uv_bin)} run python daily_pipeline.py \"$@\" 2>&1 | tee \"$pipeline_output\"",
