...
def build_managed_path(raw_path: str | None) -> str:
    fallback = "/usr/local/bin:/usr/bin:/bin"
    seen: set[str] = set()
    parts: list[str] = []
    for raw_part in str(raw_path or "").split(os.pathsep):
        part = raw_part.strip()
        if not part or part in seen:
            continue
        seen.add(part)
        parts.append(part)
    return os.pathsep.join(parts) or fallback
...
def build_env_file(
    *,
    ...
    managed_path = build_managed_path(os.getenv("PATH"))
    return "\n".join(
        [
